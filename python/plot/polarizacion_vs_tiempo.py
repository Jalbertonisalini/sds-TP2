...
def graficar(archivo_csv, archivo_salida=None):
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title(f"Evolución de $v_a$ — {Path(archivo_csv).stem}")
    ...
    if archivo_salida is None:
        ...
        archivo_salida = OUTPUT_DIR / "polarizacion_vs_tiempo.png"
    fig.savefig(archivo_salida, dpi=150)
    print(f"Gráfico: {archivo_salida}")
    print(f"va = {va_media:.4f} ± {va_std:.4f}")


if __name__ == "__main__":
    args = sys.argv[1:]
    if "-h" in args or "--help" in args:
        print("Uso: python plot/polarizacion_vs_tiempo.py --input ARCHIVO_CSV [--salida archivo.png]")
        sys.exit(0)
